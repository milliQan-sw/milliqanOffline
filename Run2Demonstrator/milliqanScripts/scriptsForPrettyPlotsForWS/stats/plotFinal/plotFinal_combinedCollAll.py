...
for datasetName in ["MilliQ","ArgoNeut","SENSEI","CMS","milliQan","milliQanRun3Both","SUBMET","luminiQPlastic","LongQuestLarge","SHIP","FORMOSA_nominal","ColliderWithMilliQ"][1:]:
    ...
    if datasetName == "SK":
        ...
        plt.loglog(dataset[:,0],dataset[:,1],color="darkcyan",linewidth=2,label="SUBMET (ongoing)")
    elif datasetName == "milliQanRun3Both":
        plt.loglog(dataset[:,0],dataset[:,1],color="blue",linewidth=2,alpha=0.5,label="Run 3 milliQan (ongoing)")
        ...
        plt.loglog(dataset[:,0],dataset[:,1],color="fuchsia",linewidth=2,label="FORMOSA (start ~2030)")
        ...
        plt.loglog(dataset[:,0],dataset[:,1],color="darksalmon",linewidth=2,label="SHIP (start ~2030)")
    elif datasetName == "luminiQPlastic":
        plt.loglog(dataset[:,0],dataset[:,1],color="darkkhaki",linewidth=2,label="FLAME LANL (start 2025)")
    elif  "LongQuest" in datasetName:
        plt.loglog(dataset[:,0],dataset[:,1],color="purple",linewidth=2,label="FLAME FNAL (start 2026)")
    else:
...
#      verticalalignment='center',transform=plt.gca().transAxes,fontsize=14)
fig.savefig("finalLimit_All.pdf",transparent=True)
