...
datasetAM = np.loadtxt("external/ArgoNeutWithMQ.csv",delimiter=",")
for datasetName in ["ArgoNeut","CMS","Collider","ColliderWithMilliQ","SENSEI","milliQan","milliQanRun3Final2","BEBC"][:-1]:
    ...
    elif datasetName == "Collider" or datasetName == "MilliQ":
        pass
    else:
        plt.loglog(dataset[:,0],dataset[:,1],color="gray",linewidth=2)
...
SENSEI_interpUp = np.interp(datasets["SENSEI"][:,0]/1000,datasets["ColliderWithMilliQ"][:,0],datasets["ColliderWithMilliQ"][:,1])
...
SENSEI_interpUp2 = np.interp(datasets["SENSEI"][:,0]/1000,datasetAM[:,0],datasetAM[:,1])
...
if "MilliQ" in datasets:
    plt.fill_betweenx(datasets["MilliQ"][:,1],0.01,datasets["MilliQ"][:,0],color="gray",alpha=0.4)
...
plt.text(0.025,0.002,"SLAC MilliQ",fontsize=9)
