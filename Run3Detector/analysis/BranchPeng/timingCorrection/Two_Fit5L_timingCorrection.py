...
def fit_histogram(hist, beamOn_true_percentage, root_file):
    ...
    combined_gaus = r.TF1("combined_gaus", "gaus(0) + gaus(3)", -23, 11) # Fit range ##############################################################################################################################
    
    # Initial parameter estimates for the two Gaussian functions
    combined_gaus.SetParameters(13, -17.5, 2.695, 59, -0.5, 5.635)  # Max Mean Stddev (left/right) ##################################################################################################################
    ...
    gaus1 = r.TF1("gaus1", "gaus", -23, -12)  # Left Range ########################################################################################################################################################
    gaus2 = r.TF1("gaus2", "gaus", -12, 11)  # Right Range #########################################################################################################################################################
    for i in range(3):
