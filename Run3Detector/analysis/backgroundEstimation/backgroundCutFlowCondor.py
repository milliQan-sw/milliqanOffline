...
@mqCut
def sidebandRMSCut(self, cutName='sidebandRMSCut', cutVal=3.0, cut=False, branches=None):

    sidebandVals = self.events['sidebandRMS'][:, :64]

    passCut = ak.any(sidebandVals > cutVal, axis=1)
    print(sidebandVals[ak.where(sidebandVals > cutVal)])
    _, passCut = ak.broadcast_arrays(self.events.npulses, passCut)
    self.events[cutName] = ~passCut
...
if __name__ == "__main__":
    ...
    setattr(milliqanCuts, 'nBarsCut', nBarsCut)
    setattr(milliqanCuts, 'nBarsDeltaTCut', nBarsDeltaTCut)
    setattr(milliqanCuts, 'sidebandRMSCut', sidebandRMSCut)
    ...
    nBarsCut = getCutMod(mycuts.nBarsCut, mycuts, 'nBarsCut', nBarsCut=4, cut=True, branches=branches)

    #require < nBars within deltaT
    nBarsDeltaTCut = getCutMod(mycuts.nBarsDeltaTCut, mycuts, 'nBarsDeltaTCut', nBarsCut=5, timeCut=100, cut=True, branches=branches)

    #sideband RMS cut
    sidebandRMSCut = getCutMod(mycuts.sidebandRMSCut, mycuts, 'sidebandRMSCut', cutVal=1.3, cut=True, branches=branches)
    ...
    #save plots
    myplotter.saveHistograms("bgCutFlow_{}_copyDem.root".format(job))
