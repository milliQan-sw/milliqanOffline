...
mass_match = re.search(r"m_(\d+p\d+)", input_path)
charge_match = re.search(r"c_(\d+p\d+)", input_path)
mass = mass_match.group(1).replace("p", ".") if mass_match else None
charge = charge_match.group(1).replace("p", ".") if charge_match else None
...
panel_hit = ak.any(inLineHits["nPE"][is_panel] > 0, axis=1)
...
#Require at least one front/back slab hit
is_slab = panellessHits["type"] == 1
slab_hit = ak.any(is_slab, axis=1)
slabHits = panellessHits[slab_hit]
n_slabHits = len(slabHits)
slabRequiredEff = round(n_slabHits / n_panelless,4) if n_panelless > 0 else 0


#Remove large hits in front/back slabs
is_slab = slabHits["type"] == 1
slab_cut = 70 
large_slab_hit = ak.any(slabHits["nPE"][is_slab] > slab_cut,axis=1)  
slablessHits = slabHits[~large_slab_hit]
n_slabless = len(slablessHits)
largeSlabHitEff = round(n_slabless / n_slabHits,4) if n_slabHits > 0 else 0
...
#PERFORM TIMING CALIBRATION: ~3ns per layer
goodRatioHits["time"] = goodRatioHits["time"] - goodRatioHits["layer"] * 3
...
signalWeights = weights[non_empty_mask][fourLayersHitMask][one_in_each_layer_mask][in_a_line][~panel_hit][slab_hit][~large_slab_hit][nPE_ratio < nPE_cut][dt < dt_cut]
...
table_data = [
    ...
    ["Require at least one front/back slab hit", f"{n_slabHits} / {n_panelless}", slabRequiredEff],
    [f"NPE < {slab_cut} in front/back slabs", f"{n_slabless} / {n_slabHits}", largeSlabHitEff],
...
with open(f"{input_path}/yield_SR2.txt","w") as outfile:
