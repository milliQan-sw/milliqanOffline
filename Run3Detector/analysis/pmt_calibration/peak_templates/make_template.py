...
h = r.TH1D(
    "h", ";pulse area [pVs];Events / 2 pVs", 125, -500, 500
)  # The ranges should probably not be set directly
...
plt.plot(
    [tstart] * 2, [np.amin(avg_spe_fix), np.amax(avg_spe_fix)], "k--"
)  # Will plot two points, for 878, at (280, min(avg_spe_fix)) and another at (280, max(avg_spe_fix))
plt.plot(
    [tend] * 2, [np.amin(avg_spe_fix), np.amax(avg_spe_fix)], "k--"
)  # Plot two point similar to previous line
...
def get_template(avg):
    imin = np.argmax(
        t_baseline > tstart
    )  # Find first value in t_baseline that is greater than tstart
    imax = np.argmax(t_baseline > tend)
    template = np.array(
        avg[imin:imax]
    )  # The template is only the values between imin and imax and averaged
...
def make_event_display(avg, vs, draw_smoothed=False, saveAs=None):
    ...
    fitted = (
        template
        * np.trapz(
            vs[i_peak - offset : i_peak - offset + template.size],
            t_baseline[: template.size],
        )
        / np.trapz(template, t_baseline[: template.size])
    )
    ...
    for a in mults:
        test = fitted * a
        sses.append(
            np.sum((vs[i_peak - offset : i_peak - offset + template.size] - test) ** 2)
        )
    ...
    plt.plot(
        t_baseline[i_peak - offset : i_peak - offset + fitted.size], fitted, "g-", lw=2
    )
...
for i in range(10):
    ...
    make_event_display(
        avg_spe_fix, vs, draw_smoothed=False, saveAs="events/event{0:03d}".format(i)
    )
...
plt.hist(
    t_peaks,
    range=[np.mean(t_peaks) - 20, np.mean(t_peaks) + 20],
    bins=40,
    histtype="step",
)
...
for i in range(10):
    ...
    make_event_display(
        ...
        vs,
        draw_smoothed=False,
        saveAs="events_tcorr/event{0:03d}".format(i),
    )
...
for freq in output_freqs:
    ...
    c.SaveAs(
        os.path.join(
            plotdir, "template_{0}_GHz.png".format(str(freq).replace(".", "p"))
        )
    )
    c.SaveAs(
        os.path.join(
            plotdir, "template_{0}_GHz.pdf".format(str(freq).replace(".", "p"))
        )
    )
