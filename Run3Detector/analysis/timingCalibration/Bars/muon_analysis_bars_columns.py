...
outfile = r.TFile('Bar_Columnar_Correction_Validated_histograms.root', 'RECREATE')
...
Panel_72_large = df.Filter('Sum(height[chan==69&&ipulse==0]>1000)')
...
for i in topbars_pan68:	
	...
	Panel_68_large =  Panel_68_large.Define(f'Time{str(i)}_ref',f'Max(timeFit[chan=={i}&&ipulse==0])')
...
for num in range(3):
	...
	if n!=8:
		...
		for i in topbars_pan68:	
			...
			Panel_68_large =  Panel_68_large.Define(f'Time{str(i+n)}',f'Max(timeFit[chan=={i+n}&&ipulse==0])')
			...
			diffHisto = Panel_68_large_.Histo1D((f'Corrected_diff_Bars{str(i)}_{str(i+n)}', 'Timing; time', 30,  -26.25, 26.25),f'TimeDiff_Bars{str(i)}_{str(i+n)}')
		...
		for i in topbars_pan68:	
			...
			if i==16 or i==17:	
				...
				Panel_68_large =  Panel_68_large.Define(f'Time{str(i+n+54)}',f'Max(timeFit[chan=={i+n+54}&&ipulse==0])')
				...
				diffHisto = Panel_68_large_.Histo1D((f'Corrected_diff_Bars{str(i)}_{str(i+n+54)}', 'Timing; time', 30,  -26.25, 26.25),f'TimeDiff_Bars{str(i)}_{str(i+n+54)}')
...
for i in topbars_pan72:
	...
	Panel_72_large =  Panel_72_large.Define(f'Time{str(i)}_ref',f'Max(timeFit[chan=={i}&&ipulse==0])')
...
for num in range(3):
	...
	for i in topbars_pan72:
		...
		Panel_72_large =  Panel_72_large.Define(f'Time{str(i+n)}',f'Max(timeFit[chan=={i+n}&&ipulse==0])')
		...
		diffHisto = Panel_72_large_.Histo1D((f'Corrected_diff_Bars{str(i)}_{str(i+n)}', 'Timing; time', 30,  -26.25, 26.25),f'TimeDiff_Bars{str(i)}_{str(i+n)}')
