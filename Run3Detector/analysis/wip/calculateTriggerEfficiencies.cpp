...
globalHist calculateRates(TFile* fout, int channel){
    ...
    TH1F* hDouble = new TH1F("hDouble", "nPE Double Coincident Hits", 1000, 0, 100);
    ...
    for(int ievent=0; ievent < c_double->GetEntries(); ievent++){
        ...
        for(int ipulse=0; ipulse < v_nPE->size(); ipulse++){
            ...
            if(this_time < 1100 || this_time > 1400) continue;

            if(this_chan == channel){ 
                primaryChannel.push_back(pair<float, float>(this_time, end_time));
                primaryNPE.push_back(v_nPE->at(ipulse));
            }
            if(this_chan == double_coincidence.first) secondary1.push_back(pair<float, float>(this_time, end_time));
            if(this_chan == double_coincidence.second) secondary2.push_back(pair<float, float>(this_time, end_time));
        }
    ...
    TH1F* hTriple = new TH1F("hTriple", "nPE Triple Coincident Hits", 1000, 0, 100);
    ...
    for(auto& cut : nPECuts){
        int countTriple = hTriple->Integral(hTriple->FindBin(cut), hTriple->GetNbinsX()+1);
        int countDouble = hDouble->Integral(hDouble->FindBin(cut), hDouble->GetNbinsX()+1);
        ...
        float ratio = 1.;
        if (rateDouble > 0) ratio = rateTriple/rateDouble;
