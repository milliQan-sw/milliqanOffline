...
#usage: python3 countingMap.py 593
...
def initializeTree(runNum):
    chain = r.TChain("t")
    chain.Add('/store/user/mcarrigan/trees/v29/MilliQan_Run' + runNum +
              '.*_v29_firstPedestals.root')
    return chain
...
def counting_map(runNum):
    ...
    rowName = [
        "front pannel", "1st row(layer one)", "2nd row(layer one)",
        "3rd row(layer one)", "4th row(layer one)", "top pannel (layer1&2)",
        "1st row(layer two)", "2nd row(layer two)", "3rd row(layer two)",
        "4th row(layer two)", "empty", "1st row(layer 3)", "2nd row(layer 3)",
        "3rd row(layer 3)", "4th row(layer 3)", "top pannel (layer3&4)",
        "1st row(layer 4)", "2nd row(layer 4)", "3rd row(layer 4)",
        "4th row(layer 4)", "back pannel"
    ]
    columnName = [
        "left pannel", "1st column", "2nd column", "3rd column", "4th column",
        "right pannel"
    ]
    c1 = r.TCanvas("c1", "demo bin labels", 10, 10, 600, 600)
    c1.SetGrid()
    c1.SetLeftMargin(0.30)
    c1.SetRightMargin(0.30)
    c1.SetBottomMargin(0.30)
    h = r.TH2F("h", "heatmap", nx, -1, nx - 1, ny, 0, ny)
    ...
    while entry < chain.GetEntries():
        ...
        # FIXME You set nx so why is this here?
        if nx < 7:
            ...
            for chainIndex, chan in enumerate(chanList):
                ...
                # Cuts placed on Data:
                # - number of photoelectrons > 40
                # - maximum pulse height > 1200
                # - Width of pulse > 200
                # - Integrated charge of pulse > 200

                if (nPEList[chainIndex] > 40.0 and heightList[
                        chainIndex] > 1200.0 and durationList[
                            chainIndex] > 200.0 and areaList[
                                chainIndex] > 200.0):
                    # if j == 1: # take away filter condition
                    row = rowlist[chainIndex]
                    column = columnlist[chainIndex]
                    layer = layerList[chainIndex]
                    print("This is the layer: ", layer)
...
def Main():
    ...
    parser.add_argument(
        'runNum',
        help='provide run number in 3 digits. we only have runs from 560-591')
