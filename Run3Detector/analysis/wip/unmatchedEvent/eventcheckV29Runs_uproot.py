...
#V 1.1 
#1/27/23 Collin
#rewrite the code with uproot.
#now the code can be run way more faster with python3
# To-do: save all TDCtime into a dictionary to make it more easier to read.
...
def count(run,variables):
    ...
    for array in uproot.iterate("/store/user/mcarrigan/trees/v29/"+ run +".*_v29_firstPedestals.root:t", ["v_groupTDC_g0"]):
    ...
    if N_unmatched ==0:
        ...
        p1 = N_repeated/N_unmatched  #number of unmatched repeated events / total number of unique unmatched event
        p2 = NumCounstructable/N_unmatched #number of construtable event / number of unique unmatched event
        N_uniqueTDC = len(uniqueTDC) #number of unique TDC time can be used for the counting the totoal number of unique event
        p3 = N_unmatched/N_uniqueTDC #number of unmatched event / number of total event
