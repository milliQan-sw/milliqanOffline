...
def parse_args():
    ...
    parser.add_argument("--slab",help="slab config",default=False,action="store_true")
    args = parser.parse_args()
    return args

def makeEventDisplays(selection,run,version,tag,maxEvents,slab):
    offlineDir = "/net/cms26/cms26r0/milliqan/Run3Offline/v{1}/MilliQan_Run{0}.*_v{1}.root".format(run,version)
    chain = r.TChain("t")
    ...
    for run,fileNumber,eventNumber in table:
        ...
        inputFile = "/net/cms26/cms26r0/milliqan/Run3/bar/{}/{}/MilliQan_Run{}.{}_default.root".format(inDir1,inDir2,run,fileNumber)
        runOfflineFactory(inputFile,"temp.root",exe,None,False,force_publish=False,database=None,appendToTag=tag,mergedTriggerFile="",drs=False,display=[eventNumber],slab=slab,runNumber=run,fileNumber=fileNumber)
