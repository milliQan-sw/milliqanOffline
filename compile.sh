...
g++ -o $NAME make_tree_temporary_for_compile.C /home/milliqan/MilliDAQ/libMilliDAQ.so `root-config --cflags --glibs` -Wno-narrowing  # same as above but with correct local file path
if [ $? -eq 0 ]; then
