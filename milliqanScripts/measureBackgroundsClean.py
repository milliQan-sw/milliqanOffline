...
tagOrig = "Full2018DatasetPlusSlabFront"
    ...
    for iC1,chan1 in enumerate(layers[1]):
        for iC2,chan2 in enumerate(layers[2]):
            for iC3,chan3 in enumerate(layers[3]):
                ...
                pathPlusSlab = tuple(sorted(set([18,chan1,chan2,chan3])))
                paths.append(path)
                paths.append(pathPlusSlab)
                if iC1 == iC2 and iC2 == iC3:
                    straight.append(path)
                    straightPlusSlab.append(pathPlusSlab)
                    if chan1 in badChans or chan2 in badChans or chan3 in badChans:
                        badStraight.append(path)
                        badStraightPlusSlab.append(pathPlusSlab)
                    else:
                        noBadStraight.append(path)
                        noBadStraightPlusSlab.append(pathPlusSlab)
                else:
                    if chan1/16 == chan2/16 and chan2/16 == chan3/16:
                        bentSameDigi.append(path)
                        bentSameDigiPlusSlab.append(pathPlusSlab)
                        if chan1 in badChans or chan2 in badChans or chan3 in badChans:
                            badBentSameDigi.append(path)
                            badBentSameDigiPlusSlab.append(pathPlusSlab)
                        else:
                            noBadBentSameDigi.append(path)
                            noBadBentSameDigiPlusSlab.append(pathPlusSlab)
                    else:
                        bentDiffDigi.append(path)
                        bentDiffDigiPlusSlab.append(pathPlusSlab)
        ...
        if abs(maxDeltaT) < 30:
            ...
            if len(pulses) == 4 and singlePulse:
                if tuple(sorted(list(chanSet))) in allPaths["StraightPlusSlab"]:
                    print "s",chanSet,tree.run,tree.file,tree.event
                elif tuple(sorted(list(chanSet))) in allPaths["BentSameDigiPlusSlab"]:
                    print "b",chanSet,tree.run,tree.file,tree.event
...
def makeABCDPredictions(inputFile,beamString,blind,nPEStrings,deltaTStrings,selections,allPaths,signalNorm):
    ...
    if blind:
        blindString = "Blind"
    outputFileABCD = r.TFile("outputFileABCDPlusSlab{0}{1}{2}.root".format(beamString,blindString,tag),"RECREATE")
            ...
            for nPEString in nPEStrings:
                ...
                for pathType in ["","bad","noBad"]:
                    ...
                    for scaleByPath in [True,False]:
                        ...
                        outputDir.cd()
                        straightPlot = inputFile.Get("{0}Vs{1}{2}/summary/{3}StraightPlusSlab_{0}_{1}{2}".format(nPEString,deltaTString,selection,pathType))
                        bentPlot = inputFile.Get("{0}Vs{1}{2}/summary/{3}BentSameDigiPlusSlab_{0}_{1}{2}".format(nPEString,deltaTString,selection,pathType))
                        straightDeltaT = straightPlot.ProjectionY()
