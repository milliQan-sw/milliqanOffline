...
void SetAddresses_old(TTree *& tree) {
...
void SetAddresses_nohs(TChain *& tree) {
...
void SetAddresses_nohs_old(TTree *& tree) {
...
vector<int> slabs = {18,20,28,22};
vector<int> allBars = {0,1,24,25,8,9,6,7,16,17,12,13,2,3,22,23,4,5};
...
string printVector(vector<float> myVector){
	string myString = "";
	for (unsigned int z=0; z<myVector.size(); z++){
		myString += tostr(myVector[z]);
		myString += ",";
...
void displayProgress(int i, int entries){
	// if(dmod((i/static_cast<double>(entries-1/100.0))*100,  0.01) != 0){ return; }
	cout  << "\r\033[1;35m[" << static_cast<int>((i*1.0)/entries*100) << "%]\033[0m";
}

// cout debugging
void debug(bool flag, int line){if(flag) cout<<"line "<<line<<" "<<endl;}
void debugVar(bool flag, int line, int var){if(flag) cout<<line<<" "<<var<<endl;}
void debugVar(bool flag, int line, double var){if(flag) cout<<line<<" "<<var<<endl;}
void debugVar(bool flag, int line, string var){if(flag) cout<<line<<" "<<var<<endl;}
void debugVar(bool flag, int line, float var){if(flag) cout<<line<<" "<<var<<endl;}
void debugVar(bool flag, int line, TString var){if(flag) cout<<line<<" "<<var<<endl;}
void debugVar(bool flag, int line, bool var){if(flag) cout<<line<<" "<<var<<endl;}
